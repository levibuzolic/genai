...
test("browser UI smoke covers filters, menus, lazy media, and stable card rendering", async (t) => {
  ...
  try {
    ...
    assert.deepEqual(await visibleTopActions(page), ["Sync & download", "More"]);
    ...
    await page.locator(".actionMenu summary").click();
    assert.deepEqual(await page.locator(".actionMenuPanel button").allTextContents(), [
      "Full rescan",
      "Download missing",
      "Retry errors",
      "Thumbnails",
    ...
    assert.ok(cardActions.height <= 34, `expected one-row card actions, got ${cardActions.height}px`);
    assert.deepEqual(await page.locator(".card").first().locator(".cardActions *").allTextContents(), [
      "Details",
      "Prompt",
      "Open"
    ]);
    ...
    assert.ok(lazyState.deferred > 0);
    ...
    const videoStayedMounted = await page.evaluate(async () => {
      ...
      return [...document.querySelectorAll(".card video")]
        .some((node) => node.dataset.stableMarker === "present");
    });
    ...
    await page.waitForSelector("#itemDialog[open]", { timeout: 5000 });
    ...
    try {
      ...
      const mobileLayout = await mobilePage.evaluate(() => {
        ...
        const firstRowCount = [...document.querySelectorAll(".card")]
          .filter((card) => Math.abs(card.getBoundingClientRect().top - firstRowTop) < 4)
          .length;
      ...
      await mobilePage.waitForSelector("#itemDialog[open]", { timeout: 5000 });
      const mobileDialog = await mobilePage.locator("#itemDialog").boundingBox();
      assert.ok(mobileDialog);
      assert.ok(mobileDialog.width >= 380, `expected mobile dialog to fill viewport, got ${mobileDialog.width}px`);
    } finally {
...
async function writeFixtureCatalog(mediaDir) {
  ...
  for (let index = 0; index < 52; index += 1) {
    ...
    if (!isMissing) {
      ...
      await writeFile(
        path.join(mediaDir, localFile),
        isVideo ? new Uint8Array([0, 0, 0, 24, 102, 116, 121, 112]) : PNG_BYTES
      );
    }
